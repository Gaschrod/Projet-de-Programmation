...
public class S_to_SE_enable{
	...
	public S_to_SE_enable(Raffinement2 m) {
		this.machine = m;
	}
	...
	/*@ 
		requires true;
 		assignable \nothing;
		ensures \result <==> machine.get_S_SE_available().apply(new Integer(1)).equals(false); */
	public boolean guard_S_to_SE_enable() {
		return machine.get_S_SE_available().apply(new Integer(1)).equals(false);
	}

	/*@ 
		requires guard_S_to_SE_enable();
		ensures guard_S_to_SE_enable() &&  machine.get_S_SE_available().equals(\old((machine.get_S_SE_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,true)))))) &&  machine.get_NE_S_available().equals(\old((machine.get_NE_S_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(2,false)))))) &&  machine.get_center_S_available().equals(\old((machine.get_center_S_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(2,false)))))) &&  machine.get_transit_S_available().equals(\old((machine.get_transit_S_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,false)))))) &&  machine.get_SW_S_available().equals(\old((machine.get_SW_S_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,false)))))); 
	 also
		requires !guard_S_to_SE_enable();
		assignable \nothing;
		ensures true; */
	public void run_S_to_SE_enable(){
		if(guard_S_to_SE_enable()) {
			...
			machine.set_S_SE_available((S_SE_available_tmp.override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,true)))));
			...
			System.out.println("S_to_SE_enable executed ");
		}
	}
