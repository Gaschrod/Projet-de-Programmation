public class center_to_transit_enable{
	...
	public center_to_transit_enable(Raffinement2 m) {
		this.machine = m;
	}
	...
	/*@ 
		requires true;
 		assignable \nothing;
		ensures \result <==> machine.get_transit_center_available().apply(new Integer(1)).equals(false); */
	public boolean guard_center_to_transit_enable() {
		return machine.get_transit_center_available().apply(new Integer(1)).equals(false);
	}

	/*@ 
		requires guard_center_to_transit_enable();
		ensures guard_center_to_transit_enable() &&  machine.get_transit_center_available().equals(\old((machine.get_transit_center_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,true)))))) &&  machine.get_center_S_available().equals(\old((machine.get_center_S_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,false)))))) &&  machine.get_center_NE_available().equals(\old((machine.get_center_NE_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,false)))))) &&  machine.get_NW_center_available().equals(\old((machine.get_NW_center_available().override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,false)))))); 
	 also
		requires !guard_center_to_transit_enable();
		assignable \nothing;
		ensures true; */
	public void run_center_to_transit_enable(){
		if(guard_center_to_transit_enable()) {
			...
			machine.set_transit_center_available((transit_center_available_tmp.override(new BRelation<Integer,Boolean>(new Pair<Integer,Boolean>(1,true)))));
			...
			System.out.println("center_to_transit_enable executed ");
		}
	}
