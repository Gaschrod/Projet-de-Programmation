...
public class spawn_train{
	...
	public boolean guard_spawn_train( Integer point_of_spawn, Integer train) {
		return ((machine.get_number_train()).compareTo(new Integer(7)) < 0 && machine.Train.has(train) && !train.equals(machine.NOTHING) && machine.Main_station.has(point_of_spawn) && machine.get_station_occupancy().apply(point_of_spawn).equals(machine.NOTHING) && !machine.get_station_occupancy().range().has(train));
	}
	...
	public void run_spawn_train( Integer point_of_spawn, Integer train){
		if(guard_spawn_train(point_of_spawn,train)) {
			...
			System.out.println("spawn_train executed point_of_spawn: " + point_of_spawn + " train: " + train + " ");
		}
	}
